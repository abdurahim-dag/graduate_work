...
class ESLoader:
    ...
    def run(self):
        ...
        self.client = es.Elasticsearch(self._settings.conn_params)

        for file_path in pathlib.Path(self._settings.dir_path).glob(
            f"**/{self._settings.src_prefix_file}-*.json"
        ):
            with closing(open(file_path, encoding='utf-8')) as file:
                body = file.read()
                self.client.bulk(index=self._settings.index_name, body=body)
