...
class ESTransform:
    ...
    def run(
        ...
        for src_file in pathlib.Path(self._settings.dir_path).glob(f"**/{self._settings.src_filename_prefix}-*.json"):
            ...
            target_file_name = re.search(f"{self._settings.src_filename_prefix}-(.*).json", src_file.name).group(1)
